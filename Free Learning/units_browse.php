...
if (!(isActionAccessible($guid, $connection2, '/modules/Free Learning/units_browse.php') == true or ($publicUnits == 'Y' and !$session->exists('username')))) {
    ...
    if ($highestAction == false) {
        ...
        $learningAreas = $unitGateway->selectLearningAreasAndCourses($session->get('gibbonPersonID'), $disableLearningAreas, $roleCategory, $session->get('gibbonSchoolYearID'), $highestAction);
        $row = $form->addRow();
